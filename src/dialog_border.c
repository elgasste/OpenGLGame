...
void DialogBorder_Render( DialogBorder_t* border, float screenX, float screenY, float scale )
{
   ...
   float fx = screenX;
   float fy = screenY + ( textMap->charSize.y * ( border->numChars.y - 1 ) );
   ...
   Blit_ImageSection( border->bordersImage, fx + ( ( border->numChars.x - 1 ) * textMap->charSize.x ), fy, (uint32_t)MenuBorderSide_TopRight * textMap->charSize.x, 0, textMap->charSize.x, textMap->charSize.y, scale );
   ...
   Blit_ImageSection( border->bordersImage, fx + ( ( border->numChars.x - 1 ) * textMap->charSize.x ), screenY, (uint32_t)MenuBorderSide_BottomRight * textMap->charSize.x, 0, textMap->charSize.x, textMap->charSize.y, scale );
   for ( y = 1; y < border->numChars.y - 1; y++ )
   {
      ...
      Blit_ImageSection( border->bordersImage, fx + ( ( border->numChars.x - 1 ) * textMap->charSize.x ), fy - ( textMap->charSize.y * y ), (uint32_t)MenuBorderSide_Right * textMap->charSize.x, 0, textMap->charSize.x, textMap->charSize.y, scale );
   }
   for ( x = 1; x < border->numChars.x - 1; x++ )
   {
   ...
   if ( border->padding > 0 )
   {
      for ( y = 1; y <= border->padding; y++ )
         for ( x = 1; x < border->numChars.x - 1; x++ ) Blit_TextChar( ' ', fx + ( textMap->charSize.x * x ), fy - ( textMap->charSize.y * y ), scale, textMap );
      for ( y = border->numChars.y - border->padding - 1; y < border->numChars.y - 1; y++ )
         for ( x = 1; x < border->numChars.x - 1; x++ ) Blit_TextChar( ' ', fx + ( textMap->charSize.x * x ), fy - ( textMap->charSize.y * y ), scale, textMap );
      for ( y = border->padding + 1; y < border->numChars.y - border->padding - 1; y++ )
      {
         for ( x = 1; x <= border->padding; x++ ) Blit_TextChar( ' ', fx + ( textMap->charSize.x * x ), fy - ( textMap->charSize.y * y ), scale, textMap );
         for ( x = border->numChars.x - border->padding - 1; x < border->numChars.x - 1; x++ ) Blit_TextChar( ' ', fx + ( textMap->charSize.x * x ), fy - ( textMap->charSize.y * y ), scale, textMap );
      }
   }
}
